        ...
        return new Promise((resolve, reject) => {
            Promise.all(
                traversal(publicDir, src => {
                    ...
                    that.upload(src, key).finally(() => {console.log(key);});
                    if (key.endsWith('index.html')) {
                        console.log("key(move index.html): " + key.substr(0, key.length - 10));
                        console.log("key(move index.html): " + key.substr(0, key.length - 11));
                        console.log("key(move index.html and add .html): " + key.substr(0, key.length - 11) + '.html');
                    }
                })
            ).then(() => {
